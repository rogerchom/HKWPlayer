...
typedef enum
{
    EPlayerState_Init,        // Initialization
    EPlayerState_Play,        // Playing
    EPlayerState_Preparing,   // Preparing
    EPlayerState_Pause,       // Paused
    EPlayerState_Stop         // Stopped
} HKPlayerState;
...
/*!
 * @brief Initializes and starts HKWirelessHD controller. This API requires a license key as string. If the input license key fails in key validation, then the API returns -1 as result. If it is successful, return 0. <p>This is a blocking call, and it will not return until the caller successfully initializes HKWireless controller. If the phone is not joining a Wi-Fi network, or any other app on the same phone is using the HKWireless controller, it waits until the app releases the controller.<p>If you needs non-blocking behavior, you should call this API asynchronously using other thread.
...
/*!
 * @brief Finds a DeviceInfo from the table by DeviceId. It is useful to retrieve DeviceInfo with a particular deviceId.
